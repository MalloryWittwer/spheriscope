...
class SphericalAutoencoder:
    def __init__(self, optimizer="rmsprop"):
        ...
        rescale_layer = layers.Rescaling(1./255)(resize_layer)  # Images are scaled to 0-1

        x = layers.Conv2D(2, (3, 3), activation="relu", padding="same", strides=4)(rescale_layer)
        ...
        x = layers.BatchNormalization()(x)
        x = layers.Conv2DTranspose(2, (3, 3), strides=2, activation="relu", padding="same")(x)
        x = layers.BatchNormalization()(x)
